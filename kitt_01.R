...
alpha = 0.8
# pal = "oslo"
gg1 <- make_dat(points = points, 
                groups = groups,
                gap = gap,
                seed = seed) %>% 
    make_art(palette = "tokyo", alpha = alpha)
gg2 <- make_dat(points = points, 
                groups = groups, 
                gap = gap,
                add_noise = TRUE, 
                seed = seed) %>% 
    make_art(palette = "berlin", alpha = alpha)
gg3 <- make_dat(points = points, 
                groups = groups,
                gap = gap,
                polygon = layergon, 
                seed = seed) %>%
    make_art(palette = "vik", alpha = alpha)
gg4 <- make_dat(points = points, 
                groups = groups,
                gap = gap,
                polygon = layergon, 
                add_noise = TRUE, 
                seed = seed) %>% 
    make_art(palette = "lisbon", alpha = alpha)
